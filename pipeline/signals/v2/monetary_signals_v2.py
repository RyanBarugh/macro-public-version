"""
Monetary Signals v2 — JPMaQS-Aligned Block Scoring
=====================================================

Reads excess/change metrics from macro.monetary_derived, scores them
...
Changes from v1:
  - Replaced core_yoy (level, neutral="mean") with excess_core_cpi (excess, neutral="zero")
  - Added F3: policy gap momentum (ry_excess changes at 3m/6m/12m)
  - All neutral="zero" — no structural bias
  - Re-scoring at every aggregation step

Factor structure (3 factors, equal weight):
    F1 (excess inflation):    excess_core_cpi        (core 6M/6M ann minus INFTEFF)
    F2 (inflation direction): nowcast_cpi_chg         (base-effects projected YoY change)
    F3 (policy gap momentum): ry_excess_3m/6m/12m_chg (change in real yield excess over r*)

All positive = hawkish/tightening = bullish for the currency.

Reads from:  macro.monetary_derived
Writes to:   macro.monetary_signals_v2

Setup SQL (run once):
---------------------
    CREATE TABLE IF NOT EXISTS macro.monetary_signals_v2 (
    ...
    CREATE INDEX IF NOT EXISTS idx_monetary_signals_v2_time
        ON macro.monetary_signals_v2(time);
    CREATE INDEX IF NOT EXISTS idx_monetary_signals_v2_currency
        ON macro.monetary_signals_v2(currency);
"""
...
SOURCE_TABLE = "monetary_derived"
TARGET_TABLE = "monetary_signals_v2"

# All positive = hawkish/tight = bullish for the currency.
# All metrics are excess or change — neutral="zero" for everything.
FACTORS = {
    "f_excess_inflation": {
        "constituents": ["excess_core_cpi"],
        "signs":        [1],
    },
    "f_inflation_direction": {
        "constituents": ["nowcast_cpi_chg"],
        "signs":        [1],
    },
    "f_policy_gap_momentum": {
        "constituents": ["ry_excess_3m_chg", "ry_excess_6m_chg", "ry_excess_12m_chg"],
        "signs":        [1, 1, 1],
    },
}
...
def _score_factor(
    ...
    All v2 constituents are excess or change — neutral="zero" for everything.
    """
    constituent_zn = {}
    ...
    for suffix, sign in zip(factor_def["constituents"], factor_def["signs"]):
        ...
        # All monetary metrics are excess/change → neutral="zero"
...
def compute_monetary_signals_v2(
    conn,
    schema: str = "macro",
) -> dict[str, pd.DataFrame]:
    """
    Full monetary block v2 scoring pipeline.
    ...
    Returns dict of all scored panels:
        ...
        "monetary_score_v2"  — final block composite (rescored if multi-factor)
    """
    logger.info("═══ Monetary Signals v2 ═══")
    ...
    if not factor_scores:
        logger.error("No valid factors — cannot compute monetary block v2 score")
        return all_panels
    ...
    all_panels["monetary_score_v2"] = block_score

    logger.info(
        "═══ Monetary block v2 score: mean|z|=%.3f, range [%.2f, %.2f] ═══",
...
def compute_and_store_monetary_signals_v2(
    conn,
    schema: str = "macro",
) -> dict[str, pd.DataFrame]:
    """
    Compute monetary block v2 signals and write to DB.
    """
    try:
        panels = compute_monetary_signals_v2(conn, schema)

        if panels:
            long_df = _panels_to_long(panels)
            _upsert(long_df, conn, schema)
            logger.info(
                "✓ Monetary signals v2 complete: %d panels, %d total rows",
        ...
        logger.error("Monetary signals v2 failed: %s", e, exc_info=True)
        raise