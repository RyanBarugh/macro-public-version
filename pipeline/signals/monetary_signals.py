"""
Monetary Signals — Flat Scoring Architecture
==============================================

Reads metrics from macro.monetary_derived, scores them using
...
Factor structure (2 factors, equal weight):
    F1 (core inflation level): core_yoy              (core CPI YoY %, neutral="mean")
...
Writes to:   macro.monetary_signals
"""
...
TARGET_TABLE = "monetary_signals"

# All positive = hawkish/tight = bullish for the currency.
FACTORS = {
    "f_core_inflation": {
        "constituents": ["core_yoy"],
        "signs":        [1],
    },
    "f_inflation_nowcast": {
...
def compute_monetary_signals(
    conn,
    schema: str = "macro",
) -> dict[str, pd.DataFrame]:
    """
    Full monetary block scoring pipeline.
    ...
    Returns dict of all scored panels:
        ...
        "monetary_score"    — final block composite (averaged, not rescored)
    """
    logger.info("═══ Monetary Signals ═══")
    ...
    if not factor_scores:
        logger.error("No valid factors — cannot compute monetary block score")
        return all_panels
    ...
    all_panels["monetary_score"] = block_score

    logger.info(
        "═══ Monetary block score: mean|z|=%.3f, range [%.2f, %.2f] ═══",
...
def compute_and_store_monetary_signals(
    conn,
    schema: str = "macro",
    lookback_days: int = 90,
) -> dict[str, pd.DataFrame]:
    """
    Compute monetary block signals and write to DB.
    ...
    try:
        panels = compute_monetary_signals(conn, schema)

        if panels:
            long_df = _panels_to_long(panels, lookback_days=lookback_days)
            _upsert(long_df, conn, schema)
            logger.info(
                "✓ Monetary signals complete: %d panels, %d total rows",
        ...
        logger.error("Monetary signals failed: %s", e, exc_info=True)
        raise